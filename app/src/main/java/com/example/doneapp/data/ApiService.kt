...
object ApiService {
  ...
  private const val BASE_URL = "https://simple-todo-list-api-kf5c.onrender.com"
  ...
  suspend fun editTarefa(
    tarefa: Task
  ): UiState<EditResponse> {
    return try {
      val response = UiState.Success(
        client.put("$BASE_URL/tarefas/${tarefa.id}") {
          contentType(ContentType.Application.Json)
          Log.d("editTarefa", tarefa.toString())
          setBody(tarefa)
        }.body<EditResponse>()
      )
      Log.d("editTarefa", response.toString())
      response
    } catch (e: Exception) {
      Log.d("editTarefaErr", e.toString())
      UiState.Error(e, "Erro")
    }
  }

  suspend fun toggleConcluida(tarefa: Task): UiState<EditResponse> {
    return try {
      val response = UiState.Success(client.patch("$BASE_URL/tarefas/${tarefa.id}") {
        contentType(ContentType.Application.Json)
        setBody(
          tarefa
        )
      }.body<EditResponse>())
      Log.d("toggleConcluida", response.toString())
      response
    } catch (e: Exception) {
      Log.d("toggleConcluidaErr", e.toString())
      UiState.Error(e, "Err")
    }
  }
